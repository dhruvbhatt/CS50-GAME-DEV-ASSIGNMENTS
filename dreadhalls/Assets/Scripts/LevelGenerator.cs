...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
		...
		mapData = GenerateMazeData();//calls the fn below which returns the value to it
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				if (mapData[z, x]) {
					...
					mapData[z, x]=true;//set to true so hole doesn't spawn at this spot
					// flag as placed so we never consider placing again
					characterPlaced = true;
				}

				// create floor and ceiling
				if(Random.Range(1,15)==1 && holes<4 && !(mapData[z, x]))
				{
