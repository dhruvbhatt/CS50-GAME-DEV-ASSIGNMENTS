...
function Level:init()
    ...
    --so the alien/player doesn't go beyond the offset. Results in some problem to reset. 
    rightWallBody = love.physics.newBody(self.world, VIRTUAL_WIDTH*2, 0, 'static')
    wallShape = love.physics.newEdgeShape(0, 0, 0, VIRTUAL_HEIGHT)
    rightWallFixture = love.physics.newFixture(rightWallBody, wallShape)
    rightWallFixture:setUserData('wall')
        ...
        if types['Obstacle'] and types['Player'] then
            ...
            if a:getUserData() == 'Obstacle' then
                ...
                if sumVel > 20 then
                    ...
                    --we store it in a temporary table before removing it, since it tries to perform collisions with other objects and will give stack overflow, if
                    --removed in the middle of a COLLISION CALLBACK. Rmoved later in the major chunk of code.
                    --always delete after world update
                end
            else
    ...
    --make t he changes required to the code upon completion of it
    self.obstacles = {}
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        if rendernew then
             xPos1, yPos1 = self.launchMarker.alien1.body:getPosition()
             xVel1, yVel1 = self.launchMarker.alien1.body:getLinearVelocity()
             xPos2, yPos2 = self.launchMarker.alien2.body:getPosition()
             xVel2, yVel2 = self.launchMarker.alien2.body:getLinearVelocity()
        end
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        if (xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5)) then
        if rendernew and ((xPos1 < 0 or (math.abs(xVel1) + math.abs(yVel1) < 1.5)) and (xPos2 < 0 or (math.abs(xVel2) + math.abs(yVel2) < 1.5))) or true then
