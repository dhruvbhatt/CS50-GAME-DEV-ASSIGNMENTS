...
function PaddleSelectState:update(dt)
    ...
    elseif keywaspressed('right') then
    ...
    if keywaspressed('return') or keywaspressed('enter') then
        gSounds['confirm']:play()
        --will only run pnce, duh. THUS LEVEL SET TO 1 
        gStateMachine:change('serve', { paddle = Paddle(self.currentPaddle),bricks = LevelMaker.createMaps(1), health = 3,score =0,
            highScores = self.highScores, level = 1, recoverPoints = 5000 })
    end
...
function PaddleSelectState:render()
    ...
    if self.currentPaddle == 1 then
        ...
        love.graphics.setColor(0.157, 0.157, 0.157, 0.5)
    end
